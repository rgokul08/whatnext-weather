...
function Home() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      ...
      setSearching(true);
      try {
        setSuggestions(await searchLocations(searchQuery, searchRequest.current.signal));
        setHighlightedSuggestion(-1);
      } catch {
        setSuggestions([]);
      } finally {
  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      try {
        selectLocation(await reverseGeocode(position.coords.latitude, position.coords.longitude), "device");
      ...
      setDetectingLocation(false);
      const message = geoError.code === 1 ? "Location access was denied. You can search manually instead." : geoError.code === 3 ? "Location detection timed out. Please search manually." : "We couldn't determine your location. Please try again or search manually.";
  ...
  const submitSearch = async () => {
    ...
    if (highlightedSuggestion >= 0 && suggestions[highlightedSuggestion]) {
      selectLocation(suggestions[highlightedSuggestion]);
      return;
    }
    setSearching(true);
    setLocationNote("Searching locations…");
    try {
      const results = await searchLocations(query);
      if (results[0]) selectLocation(results[0]);
      else setLocationNote("No matching locations found.");
    } catch {
      setLocationNote("Unable to search locations right now. Please try again.");
    } finally {
      setSearching(false);
    }
  };
