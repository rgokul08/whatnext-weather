...
import {
  Activity, AlertTriangle, ArrowDown, ArrowUp, Bike, Check, ChevronDown, ChevronRight,
  ...
  MoreHorizontal, Navigation, Plus, RefreshCw, Search, ShieldCheck, Sparkles,
  Star, Sun, Sunrise, Sunset, Thermometer, Umbrella, Waves, Wind, X, Zap, Clock3,
} from 'lucide-react';
import {
  aqiMeta, DEFAULT_LOCATION, getWeather, reverseGeocode, searchLocations, weatherGroup,
...
function freshnessLabel(value: number) {
  const seconds = Math.max(0, Math.floor((Date.now() - value) / 1000));
  if (seconds < 60) return 'Live · Updated now';
...
export default function Home() {
  ...
  const [weather, setWeather] = useState<WeatherPayload | null>(() => {
    try { return JSON.parse(localStorage.getItem(`whatnext-cache-${location.id}`) || 'null'); } catch { return null; }
  });
  ...
  const [lastUpdatedAt, setLastUpdatedAt] = useState<number>(() => Number(localStorage.getItem('whatnext-last-updated') || 0));
  ...
  const loadWeather = useCallback(async (nextLocation: LocationResult, manual = false) => {
    ...
    if (activeLocation.id !== location.id) {
      ...
      try { setWeather(cached ? JSON.parse(cached) : null); } catch { setWeather(null); }
    }
    ...
    try {
      ...
      setRecents((current) => { const next = [activeLocation, ...current.filter((item) => item.id !== activeLocation.id)].slice(0, 6); localStorage.setItem('whatnext-recents', JSON.stringify(next)); return next; });
  ...
  useEffect(() => {
    const timer = window.setInterval(() => loadWeather(location, true), 120000);
  ...
  const selectLocation = (nextLocation: LocationResult, source: 'search' | 'favorite' | 'device' = 'search') => {
    setSuggestions([]); setSearchQuery(''); setLocationNote(''); loadWeather({ ...nextLocation, source });
  };
  const useMyLocation = () => {
  ...
  const dismissLocationModal = () => { localStorage.setItem('whatnext-location-prompted', '1'); setShowLocationModal(false); };
  const toggleFavorite = () => setFavorites((items) => isFavorite ? items.filter((item) => item.id !== location.id) : [location, ...items]);
  const jumpTo = (label: string) => { const routes: Record<string, string> = { Home: '/', Forecast: '/forecast', 'Live Layer': '/map', Favorites: '/favorites', Compare: '/compare' }; window.location.href = routes[label] || '/'; };

  if (loading && !weather) return <div className="app-shell"><Header dark={dark} setDark={setDark} unit={unit} setUnit={setUnit} onMenu={() => setShowMenu(true)} onLocation={useMyLocation} /><main className="container page-content"><SkeletonDashboard /></main></div>;
  const displayWeather = weather;

  return <div className={cn('app-shell', `weather-${group}`, !dark && 'light-mode')} id="top">
    <div className="ambient ambient-one" /><div className="ambient ambient-two" />
    <AnimatePresence>{showLocationModal && <motion.div className="location-modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.section className="location-modal" initial={{ opacity: 0, y: 14, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }}><div className="location-modal-icon"><LocateFixed size={22} /></div><div className="eyebrow">Welcome to WhatNext.com</div><h2>Use your current location</h2><p>Allow WhatNext.com to show accurate local weather, sunrise, air quality, and recommendations for where you are.</p><div className="location-modal-actions"><button className="primary-action" onClick={() => { dismissLocationModal(); useMyLocation(); }}><LocateFixed size={16} /> Use My Location</button><button className="secondary-action" onClick={() => { dismissLocationModal(); searchRef.current?.focus(); }}>Search Manually</button></div><small>Your location stays in this browser and is not shared.</small></motion.section></motion.div>}</AnimatePresence>
    <Header dark={dark} setDark={setDark} unit={unit} setUnit={setUnit} onMenu={() => setShowMenu(true)} onLocation={useMyLocation} />
    <main className="container page-content">
      <div className="topline"><div><span className="eyebrow live-eyebrow"><span className="live-dot" /> {stale ? 'Cached weather' : 'Live weather intelligence'}</span><h1>Know the weather.<br /><em>Know what's next.</em></h1></div><div className="topline-meta"><span>{lastUpdatedAt ? freshnessLabel(lastUpdatedAt) : 'Waiting for live weather'}</span><button className={cn('icon-button', refreshing && 'is-spinning')} onClick={() => loadWeather(location, true)} aria-label="Refresh weather"><RefreshCw size={17} /></button></div></div>
...
function Header({ dark, setDark, unit, setUnit, onMenu, onLocation }: { dark: boolean; setDark: (value: boolean) => void; unit: 'celsius' | 'fahrenheit'; setUnit: (value: 'celsius' | 'fahrenheit') => void; onMenu: () => void; onLocation: () => void }) { return <header className="site-header"><div className="header-inner"><button className="mobile-menu" onClick={onMenu} aria-label="Open menu"><Menu size={20} /></button><Brand /><nav className="desktop-nav"><a className="active" href="/">Home</a><a href="/forecast">Forecast</a><a href="/map">Live Layer</a><a href="/favorites">Favorites</a><a href="/compare">Compare</a></nav><div className="header-actions"><button className="header-icon" onClick={onLocation} aria-label="Use current location"><LocateFixed size={17} /></button><button className="unit-toggle header-unit" onClick={() => setUnit(unit === 'celsius' ? 'fahrenheit' : 'celsius')} aria-label="Toggle temperature unit">°{unit === 'celsius' ? 'C' : 'F'}</button><button className="header-icon" onClick={() => setDark(!dark)} aria-label="Toggle theme">{dark ? <Moon size={17} /> : <Sun size={17} />}</button></div></div></header>; }
